...
export function ProductRibbonBanner({ 
  ...
  return (
    <AnimatePresence>
      {isVisible && (
        <div 
          className="absolute bottom-[92px] left-8 right-8 h-[80px] z-30 pointer-events-auto select-none cursor-pointer flex items-center justify-between"
          onDoubleClick={onDoubleClick}
        >
          {/* LAYER 1: Secondary Beyond Talks Purple Offset Accent Plate (Bottom/Behind) */}
          <motion.div
            ...
            className="absolute bottom-0 right-0 left-12 h-[38px] bg-gradient-to-r from-[#7B3FE4] via-[#9D5CFF] to-[#4A2080] rounded-br-xl shadow-[0_8px_25px_rgba(123,63,228,0.4)] border border-[#9D5CFF]/60 flex items-center justify-end px-8 -skew-x-[12deg] z-10"
          >
            {/* Un-skewed Accent Bar Content */}
            <div className="skew-x-[12deg] flex items-center gap-3 text-white font-mono font-black text-xl tracking-wider">
              <span className="bg-[#120924] text-[#9D5CFF] text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded border border-[#9D5CFF]/40">
                {badgeText || "FEATURED PRODUCT"}
              </span>
              <div className="h-4 w-px bg-white/20" />
              <span className="text-xs font-black uppercase tracking-widest text-[#E2D1FF]">SPECIAL PRICE:</span>
              <span className="text-white text-2xl font-black text-shadow">{price}</span>
            </div>
          </motion.div>

          {/* LAYER 2: Primary Skewed White Card (Top/Front) */}
          <motion.div
            ...
            className={`relative left-0 right-[280px] w-[calc(100%-260px)] h-[58px] bg-white border-l-4 border-b-2 border-[#9D5CFF] rounded-tl-xl shadow-[-8px_8px_30px_rgba(18,9,36,0.3)] z-20 flex items-center justify-between px-8 -skew-x-[12deg] overflow-visible ${className}`}
          >
            {/* Liquid Sheen Sweep Layer */}
            <div className="absolute inset-0 pointer-events-none silk-sheen-overlay z-10 rounded-tl-xl" />

            {/* Un-skewed Inner Content (keeps text and 3D thumbnail perfectly upright) */}
            <div className="skew-x-[12deg] flex items-center justify-between w-full relative z-20">
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="relative -mt-5 w-[76px] h-[76px] bg-white rounded-2xl border-2 border-[#9D5CFF] shadow-[0_8px_22px_rgba(157,92,255,0.45)] flex items-center justify-center p-1.5 z-30 shrink-0 animate-product-float">
                  {imageUrl ? (
                    ...
                    <svg viewBox="0 0 60 80" className="w-10 h-14" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <defs>
                        <linearGradient id="skewBottleGrad" x1="0" y1="1" x2="0" y2="0">
                      ...
                      <rect x="15" y="32" width="30" height="36" rx="4" fill="url(#skewBottleGrad)" />
                ...
                <div className="flex flex-col justify-center">
                  <h3 className="font-display font-black text-2xl text-[#120924] tracking-wide uppercase leading-tight">
                    {name}
                  </h3>
                  <p className="font-sans text-xs font-extrabold text-zinc-600 tracking-wide">
                    {subtext}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black uppercase tracking-widest text-[#7B3FE4]">SPOTLIGHT</span>
              </div>
